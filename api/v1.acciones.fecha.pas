...
begin
  ...
  lSql :=
  'with summary as( ' +
  '  select el.simbolo, lrt.bid_qty,lrt.bid, lrt.ask, lrt.ask_qty, datetime, ad.open, ad.high, ad.low,  lrt.last, ad.vol, ad.prev_close, lrt.mensajenro, ' +
  ...
  '  order by el.simbolo ' +
  ') ' +
  'select  s.simbolo, s.bid_qty, s.bid, s.ask, s.ask_qty, s.datetime, s.open, s.high, s.low, s.last, s.vol,s.prev_close from summary s ' +
  'where s.sm = 1 ' +
  'order by s.simbolo';
